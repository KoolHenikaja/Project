...
namespace Air_
{
    public partial class Rplus2 : Form
    {
        ...
        public void ViewTableLogR2()
        {
            string conString = "server=" + server + ";uid=" + uid + ";pwd=" + password + ";database=" + database;
            MySqlConnection con = new MySqlConnection(conString);
            con.Open();
            string viewTable = "SELECT * FROM `r2` ORDER by nomEtu ASC ";
            MySqlCommand msc = new MySqlCommand(viewTable, con);
            MySqlDataReader rd = msc.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(rd);
            TableauR2_dataGridView1.DataSource = dt;
        }
        ...
        private void rech_textBox10_TextChanged(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(conString))
            {
                try
                {
                    ...
                    string query = "SELECT * FROM `r2` WHERE `nomEtu` LIKE @search OR `prenomEtu` LIKE @search OR `parcours` LIKE @search OR   `progEtu` LIKE @search OR `CIN` LIKE @search OR `dateNaiss` LIKE @search OR `Tel` LIKE @search OR `nomLog` LIKE @search";
