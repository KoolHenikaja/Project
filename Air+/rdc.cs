...
namespace Air_
{
    public partial class rdc : Form
    {
        ...
        public void ViewTableRDC()
        {
            string conString = "server=" + server + ";uid=" + uid + ";pwd=" + password + ";database=" + database;
            MySqlConnection con = new MySqlConnection(conString);
            con.Open();
            string viewTable = "SELECT * FROM `rdc`ORDER by nomEtu ASC";
            MySqlCommand msc = new MySqlCommand(viewTable, con);
            MySqlDataReader rd = msc.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(rd);
            TableauRDC_dataGridView1.DataSource = dt;
        }
        ...
        private void rech_textBox10_TextChanged(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(conString))
            {
                try
                {
                    ...
                    string query = "SELECT * FROM `rdc` WHERE `nomEtu` LIKE @search OR `prenomEtu` LIKE @search OR `parcours` LIKE @search OR   `progEtu` LIKE @search OR `CIN` LIKE @search OR `dateNaiss` LIKE @search OR `Tel` LIKE @search OR `nomLog` LIKE @search";
